...
# Downloaded from: https://hdr.undp.org/data-center/documentation-and-downloads
# Load data
...
# Subset columns for hdi
hdi = df[[col for col in df.columns if col.startswith('hdi_1') or col.startswith('hdi_2')]]
df_s=pd.concat([df[["country"]],hdi],axis=1)

# Rename columns
df_s.columns=['country', '1990',
...
df_long = df_s.melt(id_vars='country')

# Rename columns and add to out df
...
# Subset columns
hdi_male = df[[col for col in df.columns if col.startswith('hdi_m')]]
df_s=pd.concat([df[["country"]],hdi_male],axis=1)
...
# Rename columns and add to out df
df_long.columns=["country","year","hdi_male"]
df_long["year"]=df_long["year"].astype(int)
dep_long=pd.concat([dep_long,df_long[["hdi_male"]]],axis=1)

# (3) HDI, female

# Subset columns
hdi_female = df[[col for col in df.columns if col.startswith('hdi_f')]]
df_s=pd.concat([df[["country"]],hdi_female],axis=1)
...
# Rename columns and add to out df
df_long.columns=["country","year","hdi_female"]
df_long["year"]=df_long["year"].astype(int)
dep_long=pd.concat([dep_long,df_long[["hdi_female"]]],axis=1)

# (4) Life expectancy

# Subset columns
le = df[[col for col in df.columns if col.startswith('le_1') or col.startswith('le_2')]]
df_s=pd.concat([df[["country"]],le],axis=1)
...
# Subset columns
le_male = df[[col for col in df.columns if col.startswith('le_m')]]
df_s=pd.concat([df[["country"]],le_male],axis=1)
...
# Rename columns and add to out df
df_long.columns=["country","year","lifeexp_male"]
df_long["year"]=df_long["year"].astype(int)
dep_long=pd.concat([dep_long,df_long[["lifeexp_male"]]],axis=1)

# (6) Life expectancy, female

# Subset columns
le_female = df[[col for col in df.columns if col.startswith('le_f')]]
df_s=pd.concat([df[["country"]],le_female],axis=1)
...
# Rename columns and add to out df
df_long.columns=["country","year","lifeexp_female"]
df_long["year"]=df_long["year"].astype(int)
dep_long=pd.concat([dep_long,df_long[["lifeexp_female"]]],axis=1)

# (7) Expected years of schooling

# Subset columns
eys = df[[col for col in df.columns if col.startswith('eys_1') or col.startswith('eys_2')]]
df_s=pd.concat([df[["country"]],eys],axis=1)
...
# Rename columns and add to out df
df_long.columns=["country","year","eys"]
df_long["year"]=df_long["year"].astype(int)
dep_long=pd.concat([dep_long,df_long[["eys"]]],axis=1)

# (8) Expected years of schooling, male

# Subset columns
eys_male = df[[col for col in df.columns if col.startswith('eys_m')]]
df_s=pd.concat([df[["country"]],eys_male],axis=1)
...
# Rename columns and add to out df
df_long.columns=["country","year","eys_male"]
df_long["year"]=df_long["year"].astype(int)
dep_long=pd.concat([dep_long,df_long[["eys_male"]]],axis=1)

# (9) Expected years of schooling, female

# Subset columns
eys_female = df[[col for col in df.columns if col.startswith('eys_f')]]
df_s=pd.concat([df[["country"]],eys_female],axis=1)
...
# Rename columns and add to out df
df_long.columns=["country","year","eys_female"]
df_long["year"]=df_long["year"].astype(int)
dep_long=pd.concat([dep_long,df_long[["eys_female"]]],axis=1)

# (10) Mean years of schooling

# Subset columns
mys = df[[col for col in df.columns if col.startswith('mys_1') or col.startswith('mys_2')]]
df_s=pd.concat([df[["country"]],mys],axis=1)
...
# Rename columns and add to out df
df_long.columns=["country","year","mys"]
df_long["year"]=df_long["year"].astype(int)
dep_long=pd.concat([dep_long,df_long[["mys"]]],axis=1)

# (11) Mean years of schooling, male

# Subset columns
mys_male = df[[col for col in df.columns if col.startswith('mys_m')]]
df_s=pd.concat([df[["country"]],mys_male],axis=1)
...
# Rename columns and add to out df
df_long.columns=["country","year","mys_male"]
df_long["year"]=df_long["year"].astype(int)
dep_long=pd.concat([dep_long,df_long[["mys_male"]]],axis=1)

# (12) Mean years of schooling, female

# Subset columns
mys_female = df[[col for col in df.columns if col.startswith('mys_f')]]
df_s=pd.concat([df[["country"]],mys_female],axis=1)
...
# Rename columns and add to out df
df_long.columns=["country","year","mys_female"]
df_long["year"]=df_long["year"].astype(int)
dep_long=pd.concat([dep_long,df_long[["mys_female"]]],axis=1)

# Add country codes manually and make sure no countries get missing
...
dep_long.loc[dep_long["country"]=="Andorra","gw_codes"]=232
...
dep_long.loc[dep_long["country"]=="Liechtenstein","gw_codes"]=223
...
dep_long.loc[dep_long["country"]=="Marshall Islands","gw_codes"]=983
dep_long.loc[dep_long["country"]=="Monaco","gw_codes"]=221
...
dep_long.loc[dep_long["country"]=="Palau","gw_codes"]=986
...
dep_long.loc[dep_long["country"]=="Samoa","gw_codes"]=990
dep_long.loc[dep_long["country"]=="San Marino","gw_codes"]=331
...
dep_long=dep_long[['country','gw_codes','year','hdi','hdi_male','hdi_female','lifeexp','lifeexp_male','lifeexp_female','eys','eys_male','eys_female','mys','mys_male','mys_female']]
dep_long.to_csv("data_out/hdi_cy.csv",sep=',')
print(dep_long.duplicated(subset=['year',"country","gw_codes"]).any())
print(dep_long.duplicated(subset=['year',"country"]).any())
print(dep_long.duplicated(subset=['year',"gw_codes"]).any())
dep_long.dtypes
