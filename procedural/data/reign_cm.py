...
duplicates = reign.duplicated(subset=['country', 'dd'],keep=False)
...
# Note Grenada 1983-10 is coded three times with tenure_months=56,0,1 --> remove both 0 and 1
reign=reign[~reign.index.isin(duplicate_rows.loc[(duplicate_rows["tenure_months"]==1) | (duplicate_rows["tenure_months"]==0)].index)]
...
reign=reign.sort_values(by=['gw_codes', 'dd'])
reign=reign.reset_index(drop=True)
reign.to_csv("data_out/reign_cm.csv",sep=',')
print(reign.duplicated(subset=['dd',"country","gw_codes"]).any())
print(reign.duplicated(subset=['dd',"country"]).any())
print(reign.duplicated(subset=['dd',"gw_codes"]).any())
reign.dtypes # dd --> period[M] but will become object outside of python
