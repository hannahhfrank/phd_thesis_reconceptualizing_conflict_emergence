...
# Load data for 'Ruggedness: The blessing of bad geography in Africa'
# Download from: https://diegopuga.org/data/rugged/
...
# Obtain country codes manually (left GW codes, and right codes in rugged data)
...
"Kosovo":[347,99999], ####<----- match with Serbia and Montenegro
"Kuwait":[690,414],
...
"Monaco":[221,492],
"Mongolia":[712,496],
"Montenegro":[341,99999], ###<----- match with Serbia and Montenegro
"Morocco":[600,504],
"Mozambique":[541,508],
...
"Senegal":[433,686],
"Serbia":[340,99999], ###<------Serbia and Montenegro in rugged data
"Seychelles":[591,690],
...
"South Sudan":[626,99999], ###<------- match with Sudan
"Spain":[230,724],
...
"Yugoslavia":[345,891], ###<------Serbia and Montenegro in rugged data
"Zambia":[551,894],
...
# Merge country codes
rug_s=pd.merge(left=rug_s,right=df_ccodes[['gw_codes','isonum']],on=["isonum"],how="left")                       
rug_s=rug_s[["country","gw_codes","rugged","land_area","soil","desert","tropical","cont_africa","cont_asia"]]

# Drop countries which are in rugged data but not in Gleditsch and Ward
rug_s = rug_s.dropna(subset=["gw_codes"])

# Add countries from Gleditsch and Ward that are missing

# Define add df
df_add=pd.DataFrame()

# Loop through missing countries
for c in ["Abkhazia","Czechoslovakia","German Democratic Republic","Kosovo","Montenegro","South Sudan","South Ossetia","Yemen, People's Republic of","Serbia"]:
   # Subset data to add
    s = {'country': c,
...
# Merge with rugged data, sort and reset index
rug_s = pd.concat([rug_s,df_add])
rug_s = rug_s.sort_values(by=["country"])
rug_s = rug_s.reset_index(drop=True)

# Manually "impute" missing countries with 'closest' matches
...
rug_s.loc[rug_s["country"]=="Serbia", "rugged"]=rug_s["rugged"].loc[rug_s["country"]=="Serbia and Montenegro"].values[0]
rug_s.loc[rug_s["country"]=="Serbia", "soil"]=rug_s["soil"].loc[rug_s["country"]=="Serbia and Montenegro"].values[0]
rug_s.loc[rug_s["country"]=="Serbia", "desert"]=rug_s["desert"].loc[rug_s["country"]=="Serbia and Montenegro"].values[0]
rug_s.loc[rug_s["country"]=="Serbia", "tropical"]=rug_s["tropical"].loc[rug_s["country"]=="Serbia and Montenegro"].values[0]
...
# Get data on the country-year level 

# This step does not account for the start dates of countries (e.g., South Sudan)
# and the data should always be merged with the appropriate sample.
...
# Save
df_agg['gw_codes']=df_agg['gw_codes'].astype('int64')
