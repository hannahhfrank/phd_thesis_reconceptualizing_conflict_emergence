...
# Manually downloaded from: https://climateknowledgeportal.worldbank.org/download-data
# API call https://cckpapi.worldbank.org/cckp/v1/era5-x0.25_timeseries_tas_timeseries_annual_1950-2022_mean_historical_era5_x0.25_mean/all_countries_subnationals?_format=json

# Load data (manually convert from excel into csv)
#df = pd.read_excel("temp.xlsx")
df = pd.read_csv("temp.csv")

# Subset to only include country level 
df_s = df[df['code'].str.len() == 3]

# Manually remove some territories because these are coded under the country name
# (e.g., Taiwan is coded as China), which produces duplicates below
df_s = df_s[df_s["code"]!="TWN"] # Taiwan (Taiwan is not included in sample)
df_s = df_s[df_s["code"]!="ALA"] # Aland
df_s = df_s[df_s["code"]!="GLP"] # Guadeloupe
df_s = df_s[df_s["code"]!="GUF"] # French Guiana
df_s = df_s[df_s["code"]!="MTQ"] # Martinique
df_s = df_s[df_s["code"]!="MYT"] # Mayotte
df_s = df_s[df_s["code"]!="REU"] # Reunion
...
# Rename columns and subset years
...
dep_long.loc[dep_long["country"]=="Federated States of Micronesia","gw_codes"]=987
...
# Check if any codes are missing then remove
